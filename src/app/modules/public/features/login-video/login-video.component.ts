    ...
    if (this.auth.user['verified'] && this.auth.user['verified'] === 1) {
      if (this.auth.isLoggedIn()) {
        ...
        if (this.auth.isLoggedIn()) {
          ...
          switch (true) {
            case this.auth.roles.some(r => r.name === 'admin_hsk') :
          ...
          // const redirect = this.params && this.params.hasOwnProperty('redirect') ? this.params['redirect'] : (thisinh ? APP_CONFIGS.defaultRedirect :(doitac ? 'admin/doitac' : '/admin/dashboard'));
          const pageTitle = APP_CONFIGS.pageTitle || 'admin area';
          this.title.setTitle(pageTitle);
          await this.router.navigate([redirect], {queryParams: this.params});
          this.loading = false;
