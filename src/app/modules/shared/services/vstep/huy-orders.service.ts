...
export class HuyOrdersService {
  private api = getRoute('huy-orders/');
  constructor(
  ...
  getDataByOrderId(order_id: number): Observable<HuyOrders> {
  ...
  getDataByOrderIdAndType(order_id: number,type:string): Observable<HuyOrders[]> {
    const conditions: OvicConditionParam[] = [
      {
        conditionName: 'order_id',
        condition: OvicQueryCondition.equal,
  ...
  getDataByPageAnd(page :number, search: string ,kekhach_id?: number ,diemduthi_id?:number,type?:string): Observable<{ recordsTotal: number, data: HuyOrders[] }> {
    const conditions: OvicConditionParam[] = [
      {
        conditionName: 'type',
        condition: OvicQueryCondition.equal,
  ...
  getDataUnlimitByKehoachIdAndCapHsk(kekhach_id?: number ,caphsk_id?:number,type?:string): Observable<{ recordsTotal: number, data: HuyOrders[] }> {
    const conditions: OvicConditionParam[] = [
    ];
    if(kekhach_id){
      conditions.push({
