...
export class ExpThisinhDuthiService {
  ...
  exportHosoLuuTru(object:any, title:string, fileName:string,header:any, headerName:string ) {

    const wb = new Workbook();
    const worksheet = wb.addWorksheet('Danh sách thí sinh', { pageSetup: { paperSize: 9, orientation: 'portrait' } });

    const text_header = headerName ;
    worksheet.addRow([text_header]);
    worksheet.addRow([""]);
    ...
    this.setCellProperties(worksheet.getCell('O3'), 14, { bold: true });
    worksheet.mergeCells('P3:P4');
    this.setCellProperties(worksheet.getCell('P3'), 14, { bold: true });
    worksheet.mergeCells('Q3:Q4');
    this.setCellProperties(worksheet.getCell('Q3'), 14, { bold: true });
