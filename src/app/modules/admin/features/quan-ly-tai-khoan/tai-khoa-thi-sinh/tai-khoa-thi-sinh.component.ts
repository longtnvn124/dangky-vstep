...
@Component({
  selector: 'app-tai-khoa-thi-sinh',
  templateUrl: './tai-khoa-thi-sinh.component.html',
  styleUrls: ['./tai-khoa-thi-sinh.component.css']
})
export class TaiKhoaThiSinhComponent implements OnInit {
    ...
    this.userService.getListUser(this.themeSettingsService.settings.rows, this.page, this.search,this.role_use).subscribe({
      next: ({recordsTotal,data}) => {
        ...
        this.data = data.map(u => {
          ...
          u['__verified'] = u.verified === 1 ? `<span class="veri veri-access"> Đã xác nhận </span>` : `<span <span class="veri veri-danger">Chưa xác nhận</span>` ;
  ...
  editUser(user) {
    if (user) {
      ...
      this.f['password'].setValue(user.password ? user.password : '');
      ...
      this.defaultPass = user.password ? user.password:'';
    }
  ...
  creatUserChangeVerified(value: number) {
  ...
  taoTaiKhoan(form: FormGroup) {
    ...
    if (form.valid) {
      ...
      data.role_ids = [this.role_use];
      if (data.role_ids.some(r => r === 6)) {
        data.display_name.split(' ');
        this.notificationService.isProcessing(true);
        this.registerUserService.creatUser(data).subscribe(
          {
            next: () => {
              this.resetForm(this.formSave);
  ...
  resetForm(form: FormGroup) {
    form.reset({
      ...
      email: '',
      password: '',
      role_ids: this.role_use ? [this.role_use]: [],
      status: 1,
      verified: 1,
    });
  }
  ...
  sendEmailActive(item:User){
    ...
    this.registerAccountService.VerifiCationAccount(item.id,dataParram).subscribe({
      next:()=>{
        this.notificationService.toastSuccess('Gửi Email thành công');
        this.notificationService.isProcessing(false);
      },
      error:()=>{
        this.notificationService.toastError('Gửi Email không thành công');
        this.notificationService.isProcessing(false);
      }
    })
  }
