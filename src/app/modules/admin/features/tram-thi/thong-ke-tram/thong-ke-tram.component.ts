...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ButtonModule, DropdownModule, FormsModule, InputTextModule, MatProgressBarModule, ReactiveFormsModule, RippleModule, SharedModule, SharedModule, TableModule, TooltipModule, ThisinhInfoComponent, FormUpdateUserComponent],
...
export class ThongKeTramComponent implements OnInit {
  ...
  @ViewChild('formUser', { static: true }) formUser: TemplateRef<any>;
  ...
  btnViewInfo(item: OrdersVstep) {
    // console.log(item);

    this.userSelect = { ...item['user'], id: item.user_id }
  ...
  btnViewaccout(item: OrdersVstep) {
    // console.log(item);
    this.userSelect = { ...item['user'], id: item.user_id }
    this.notifi.openSideNavigationMenu({
      offsetTop: '0px',
      size: 600,
      template: this.formUser
    })
  }
