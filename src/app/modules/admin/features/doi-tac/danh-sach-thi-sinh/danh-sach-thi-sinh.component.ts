...
export class DanhSachThiSinhComponent implements OnInit {
  ...
  @ViewChild('formAddImgForThisinh', {static: true}) formAddImgForThisinh: TemplateRef<any>;
  @ViewChild('formAddImgCCCD', {static: true}) formAddImgCCCD: TemplateRef<any>;
  ...
  loadData(page: number, search ?: string) {
    ...
    this.thisinhInfoService.getDataBycreatedby(page, search).subscribe({
  ...
  btnViewAddImage(){
    this.notificationService.openSideNavigationMenu({
      name:'name',
      template: this.formAddImgForThisinh,
      size: this.sizeFullWidth,
      offsetTop: '0px'
    });
  }
