...
export class ThongTinTaiKhoanComponent implements OnInit {
	...
	handleSubmitEvent() {
      ...
      // this.profileService.update(data).pipe(switchMap(() => this.userService.getUser(this.auth.user.id))).subscribe({
      this.profileService.update(data).pipe(switchMap(() => this.profileService.get())).subscribe({
				next: (user) => {
					this.auth.updateUser(user);
					this.notificationService.isProcessing(false);
