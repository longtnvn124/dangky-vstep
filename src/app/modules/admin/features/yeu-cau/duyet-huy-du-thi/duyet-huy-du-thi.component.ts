...
@Component({
  selector: 'app-duyet-huy-du-thi',
  templateUrl: './duyet-huy-du-thi.component.html',
  styleUrls: ['./duyet-huy-du-thi.component.css']
})
export class DuyetHuyDuThiComponent implements OnInit {
    ...
    if (this.kehoach_id !== 0) {
      ...
      this.ordersService.getDataBykehoachIdAndSelectforThongke(this.kehoach_id,'id,kehoach_id,caphsk_id,lephithi,trangthai_thanhtoan,user_id,parent_id,created_by,updated_by')
        .pipe(switchMap(m=>{
          ...
          return forkJoin([of(m),this.ordersService.getDataByparentIds(idsParent,'id,kehoach_id,caphsk_id,lephithi,trangthai_thanhtoan,user_id,parent_id,created_by,updated_by')]);
        }))
        .subscribe({
          next:([dataOrder,dataParent])=>{
            const dataMap = dataOrder.map((m,index)=>{
              ...
              m['__index'] = index + 1;
              m['__madk'] = 'hsk' + m.id;
              m['__status_converted'] = m.trangthai_thanhtoan === 1 ? 'Thanh toán thành công': 'Chưa thanh toán';
  ...
  async clickHuyDuThi(item:OrdersHsk){

    const button = await this.notifi.confirmRounded('Xác nhận thanh hủy dự thi với thí sinh ' + item['__thisinh_hoten'],'XÁC NHẬN',  [BUTTON_YES, BUTTON_NO]);
    if (button.name === BUTTON_YES.name) {
      const object= {
        ...
        order_id :item.id,
        hoten: item['__thisinh_hoten']
      }
      this.notifi.isProcessing(true);
      this.hskHuyOrderService.create(object).subscribe({
        next:()=>{
          this.notifi.toastSuccess("Hủy dự thi thành công");
          this.notifi.isProcessing(false);
          this.loadData(this.page,this.kehoach_id,this.search);
        },error:()=>{
          this.notifi.isProcessing(false);
