...
export class HoanComponent implements OnInit {
  ...
  loadData(page: number, kehoach_id?: number, search?: string) {
    ...
    this.huyOrdersService.getDataByPageAnd(page, this.search, this.kehoach_id, this.caphsk_id_select, 'change').subscribe(
      {
  ...
  btnExportExcelHskHuyDk() {
    if (this.kehoach_id) {
      ...
      this.huyOrdersService.getDataUnlimitByKehoachIdAndCapHsk(this.kehoach_id, this.caphsk_id_select,'change').subscribe(
        {
          next: ({data, recordsTotal}) => {
            this.recordsTotal = recordsTotal;
  ...
  private  loopActiveChangeDotthiByApi(data: HuyOrders[],step:number,percent: number):Observable<HuyOrders[]>{
    ...
    if(index !== -1){
      ...
      return  this.huyOrdersService.ActiveChangeDotthi(item.id).pipe(switchMap(m=>{
  ...
  reNameKehoachByid(id:number){
    return this.dsKehoachthi.find(f=>f.id === id) ? this.dsKehoachthi.find(f=>f.id === id).title : '';
  }
}
