...
export class HuyComponent implements OnInit {
  ...
  loadData(page: number, kehoach_id?: number, search?: string) {
    ...
    this.hskHuyOrderService.getDataByPageAnd(page, this.search, this.kehoach_id, this.caphsk_id_select, 'cancel').subscribe(
      {
        next: ({data, recordsTotal}) => {
          ...
          this.listData = data.length > 0 ? data.map((m, index) => {
            ...
            m['_status'] = m.state;
            m['_cccd_img_truoc'] = this.fileService.getPreviewLinkLocalFileNotToken(JSON.parse(thisinh['cccd_img_truoc'])[0]);
            m['_cccd_img_sau'] = this.fileService.getPreviewLinkLocalFileNotToken(JSON.parse(thisinh['cccd_img_sau'])[0]) ;

            m['_files'] = m['files'] !== null ? this.fileService.getPreviewLinkLocalFileNotToken(m['files'][0]) : '';
            m['_minhchung'] = m['minhchung'] !== null ? this.fileService.getPreviewLinkLocalFileNotToken(m['minhchung'][0]) : '';
  ...
  btnExportExcelHskHuyDk() {
    if (this.kehoach_id) {
      ...
      this.hskHuyOrderService.getDataUnlimitByKehoachIdAndCapHsk(this.kehoach_id, this.caphsk_id_select,'cancel').subscribe(
        {
          next: ({data, recordsTotal}) => {
            this.recordsTotal = recordsTotal;
  ...
  private loopDelete(data:HskHuyOrder[], step: number, percent: number):Observable<HskHuyOrder[]>{
    const index = data.findIndex(i => !i['isDelete']);
    if(index !== -1){
      const newPercent: number = percent + step;
      this.notifi.loadingAnimationV2({process: {percent: newPercent}});
      return this.hskHuyOrderService.delete(data[index].id).pipe(switchMap(m=>{
