...
export class InterceptorsService {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (request.url.startsWith(getHost())) {
      ...
      const _moment: Moment = moment(new Date());
      const none: string = (['POST', 'PUT'].includes(request.method.toUpperCase()) ? JSON.stringify(request.body) : '') + X_APP_ID + _moment.format('YYYY-MM-DD HH:mm:00');
      ...
      if (!request.params.get('token') && !request.url.endsWith('refresh-token') && token) {
