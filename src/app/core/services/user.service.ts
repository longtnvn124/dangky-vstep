...
export class UserService {
  ...
  getListUser(rows:number,  page: number, search?: string, role_ids?:string): Observable<{ recordsTotal: number, data: User[] }> {
    const conditions: OvicConditionParam[] = [
    ];
  ...
  getUserByUsername(username:string,select :string):Observable<User>{
    const conditions: OvicConditionParam[] = [
      {
        conditionName: 'username',
        condition: OvicQueryCondition.equal,
  ...
  queryUserByListUserName(userNameList:string[],createdBy:number,select:string): Observable<User[]> {
    const fromObject = {
      limit: -1,
      include:userNameList.join(','),
      include_by:'username' ,
      select:select ? select:''
    };
    const conditions: OvicConditionParam[] = [
  ...
  queryUserByIds(ids: number[],select:string): Observable<User[]> {
    const fromObject = {
      limit: -1,
      include:ids.join(','),
      include_by:'id' ,
    ...
    const params = this.appHttpParamsService.paramsConditionBuilder(conditions, new HttpParams({fromObject}));
    return this.http.get<Dto>(this.api, {params}).pipe(map(res => res.data.length>0 ? res.data: [] ));
  }
