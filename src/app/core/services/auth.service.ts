...
import {SysConfigsService, SystemConfig} from "@shared/services/sys-configs.service";
import {BUTTON_CONFIRMED, BUTTON_UPDATE} from "@core/models/buttons";
...
export class AuthService {
  ...
  public checkAppVersion() {
    ...
    if (Number.isNaN(hash) || hash < (currentTime - fiveMinutes) && hash > (currentTime + fiveMinutes)) {
      ...
      const requireVersion: number = this.getSysConfigValue('__APP_VERSION', appVersion);
