...
export class FileService {
   ...
   * ********************************************************/
  private static packFiles(files: File[], status: number = 0, tag?:string): FormData {
  ...
  uploadMultipleFilesByhsk(files: File[], _public:number,tag?:string): Observable<OvicFile[]> {
    // const params = new HttpParams().set('donvi_id', donvi_id.toString()).set('user_id', user_id.toString());
    return this.http.post<Dto>(this.media, FileService.packFiles(files,_public,tag)).pipe(
      retry(2),
      map(res => res.data)
    );
  }
